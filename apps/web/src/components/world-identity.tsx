...
export function WorldIdentity({ client, signedIn, address }: { client?: AccordClient; signedIn: boolean; address?: Address }) {
  ...
  return <section className="identity-section" aria-labelledby="world-heading">
    ...
    <div className="identity-copy"><h2 id="world-heading">Verify with World ID</h2><p>{!signedIn ? "Sign in with your wallet to connect a World ID session." : status.isPending ? "Checking your verification status…" : status.isError ? "Verification status is temporarily unavailable." : !status.data?.configured ? "World ID verification is temporarily unavailable." : status.data.enrolled ? "Your wallet is linked. Each claim asks for a fresh verification." : "Link your identity to this wallet before your first claim."}</p>{message && <p className="identity-message" role="status">{message}</p>}</div>
    <div className="identity-action">{status.data?.enrolled && <span className="identity-linked"><ShieldCheck size={15} /> Identity linked</span>}<Button onClick={begin} disabled={!signedIn || !status.data?.configured || busy} variant="outline">{busy ? "Preparing…" : status.data?.enrolled ? "Confirm again" : "Connect World ID"} <ArrowRight size={15} /></Button></div>
