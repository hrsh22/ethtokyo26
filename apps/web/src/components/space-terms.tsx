...
export function SpaceTerms({ spaceAddress, decimals, account, symbol = "tokens", audience = "public", disabled = false, onAction }: {
  ...
  return <section className="space-console__terms" aria-label={heading}>
    ...
    {terms.isPending ? <p className="space-console__terms-status" role="status">Loading allocations…</p>
      ...
      : <div className="space-console__terms-list">{allocations.toReversed().map((entry) => {
        const { id, allocation, mandate } = entry;
        ...
        const state = closed ? "Closed" : !funded ? "Fully used" : isAgent && !mandateActive ? "Needs mandate" : "Active";
        return <article className={`space-console__term${yours ? " is-yours" : ""}`} key={id.toString()}>
          <div className="allocation-identity"><span className={`allocation-icon${isAgent ? " is-agent" : ""}`}>{isAgent ? <Bot size={21} /> : <UserRound size={21} />}</span><div><h3>Allocation {id.toString()}</h3><p>{isAgent ? "Agent budget" : "Personal allocation"}</p></div><span className={`allocation-status${closed || !funded ? " is-muted" : ""}`}>{state}</span></div>
          ...
          <p className="allocation-condition">{closed ? "Unspent funds returned to the owner." : isAgent ? mandateActive ? "Payments follow the mandate’s daily limit and screening checks." : "The owner must set an active mandate before this agent can pay." : "World ID verification is required for each claim."}</p>
