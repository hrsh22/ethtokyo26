  ...
  return <Tabs.Root ref={root} className="space-workspace" value={view} onValueChange={(next) => { setView(next); onAction(null); }}>
    ...
    {["manage", "mine"].filter((role) => role !== "manage" || isOwner).map((role) => <Tabs.Content value={role} key={role} className={`space-perspective space-perspective--${role}`}>
      ...
      {actionInfo ? <>
        ...
        {role === "manage" ? <div className="space-owner-tools"><div><h3>Owner tools</h3><p>Permissions and recovery stay in your control.</p></div><Button variant="outline" onClick={() => onAction("mandate")}><Bot size={16} />Agent mandates</Button><Button variant="ghost" onClick={() => onAction("settings")}><Settings2 size={16} />Manage access</Button></div> : null}
        {role === "mine" ? <details className="space-known-allocation"><summary>Have an allocation ID?</summary><p>Use an older allocation, or retry if it hasn’t appeared yet. Your wallet must still have permission.</p><div><Button variant="outline" onClick={() => onAction("claim")}>Claim by allocation ID</Button><Button variant="outline" onClick={() => onAction("payments")}>Pay by allocation ID</Button></div></details> : null}
      </>}
    </Tabs.Content>)}
