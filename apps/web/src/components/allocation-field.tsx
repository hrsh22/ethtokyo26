...
export function AllocationField({ spaceAddress, account, purpose, selectedId = "" }: {
  spaceAddress: string; account: string; purpose: Purpose; selectedId?: string;
}) {
  ...
  return <div className="allocation-field"><Label className="space-console__field"><span>Allocation</span>{useManual ? <Input name="allocationId" inputMode="numeric" pattern="[1-9][0-9]*" required defaultValue={selectedId} placeholder="Enter allocation ID" onChange={() => setManual(true)} /> : <select name="allocationId" defaultValue={selectedExists ? selectedId : ""} required><option value="" disabled>Select an allocation</option>{options.map(({ id, allocation }) => <option key={id.toString()} value={id.toString()}>Allocation {id.toString()} · {allocation[0] === zeroAddress ? "Agent budget" : `${allocation[0].slice(0, 6)}…${allocation[0].slice(-4)}`}</option>)}</select>}</Label>{options.length > 0 && !selectedId ? <button className="allocation-field__toggle" type="button" onClick={() => setManual(!manual)}>{manual ? "Choose from your allocations" : "Use an allocation ID instead"}</button> : null}</div>;
}
