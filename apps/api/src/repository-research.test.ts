...
it("delivers sourced evidence, exposes truncated coverage and caches the same snapshot", async () => {
  ...
  const report = await repositoryReport(["fixture/tool"], "comparison", ["TypeScript", "webhooks"]);
  ...
  expect((await repositoryReport(["fixture/tool"], "comparison", ["TypeScript", "webhooks"])).collectedAt).toBe(report.collectedAt);
