...
The web app is at `http://localhost:3000`; its `/api` path proxies to the Effect API on port 4000. Set `NEXT_PUBLIC_REOWN_PROJECT_ID` in the root `.env` to a project ID from the [Reown Dashboard](https://dashboard.reown.com/) and restart the web app to enable Reown AppKit. **Connect wallet** opens a wallet picker for installed browser wallets and WalletConnect. To sign in with the local demo human wallet, choose WalletConnect and copy its pairing link, then run `pnpm wallet:demo` in a terminal and paste the link when prompted. This test wallet reads `DEMO_HUMAN_PRIVATE_KEY` locally, accepts only Accord on localhost and Ethereum Sepolia, and signs wallet login messages; it does not submit transactions. The API sponsor pays Sepolia network fees for actions in new tUSDC Spaces; users sign messages but need no ETH. `/` is a short landing page and `/spaces` is your home; **Demo** opens the public read-only Space. **Connect wallet** connects and then asks for the sign-in message in one step. **New Space** names the Space and deploys it with tUSDC in one sponsored transaction; an interrupted deployment can be finished from the home page without paying again. The **Get 1,000 tUSDC** button on Your Spaces and in New Space setup mints free demo tokens to the connected wallet and can be used repeatedly. A Space page shows its people and agents as tiles next to its onchain activity. **Give someone a budget** covers both kinds: a person's allowance (ENS name or address, total and schedule) or an agent's budget, where the agent wallet is read from its ENS name and approve, fund and mandate run as one tracked sequence. Each allocation has its own page at `/spaces/<address>/a/<id>`, which is the link you share: the named person claims there with World ID, the agent wallet pays there, and the owner shares, edits the mandate, revokes, or closes and recovers from the same page. Allowances assigned to your wallet appear under **Shared with you** on the home page; public Space and allocation links still open without signing in. The Space console reads current allocation and mandate terms from Sepolia before a person claims or an agent pays. Owners can revoke agent mandates or close an allocation and recover its unspent tokens; the recovery right appears in the public terms. The API config endpoint supplies the trusted factory, adapter, and permit signer addresses. An activated Space requires a deployed factory, adapter, and token configured in `.env`.
...
The current Sepolia deployment uses [AccordForwarder](https://sepolia.etherscan.io/address/0x947f24d2749f00be27062cec447457cac3c7a6e0), [SpaceFactory](https://sepolia.etherscan.io/address/0x3ccbb42840e290e38f4f10040e08140179dfa0a9), and the six-decimal [tUSDC test token](https://sepolia.etherscan.io/address/0x06729abbe1b9683ea3d5addd151c15c93be2a0a4). The API pays gas from `SPONSOR_PRIVATE_KEY` for wallet-signed actions in new Spaces and sends repeatable 1,000 tUSDC faucet claims. Earlier ACD contracts remain on Sepolia, but their records and pages are no longer supported by this app. The [public tUSDC demo Space](https://accord.hrsh.dev/spaces/0x3da905211FD0Ed5D8E10203FAF47a757d6022F0D) has separate owner, beneficiary, and ENS-authorized agent wallets.
